...
public class SigrunTjeneste {
    ...
    private List<PgiFolketrygdenResponse> pensjonsgivendeInntektForFolketrygden(String fnr, IntervallEntitet opplysningsperiode) {
        ...
        List<Future<Optional<PgiFolketrygdenResponse>>> futures = new ArrayList<>();
        try (ExecutorService executorService = Executors.newVirtualThreadPerTaskExecutor()) {
            utledTidligereÅr(opplysningsperiode, senesteÅr, svarSenesteÅr.isPresent()).forEach(år -> futures.add(
                systemuserThreadLogin.submitToExecutorService(executorService, () ->  sigrunConsumer.hentPensjonsgivendeInntektForFolketrygden(fnr, år))));
        }
        for (Future<Optional<PgiFolketrygdenResponse>> future : futures) {
            try {
                future.get().ifPresent(svarene::add);
